...
volatile bool onTargetA  = false;
// const uint8_t offTargetA = 10;    // Off Target A Light
volatile bool offTargetA = false;
// const uint8_t offTargetB = 11;    // Off Target B Light
volatile bool offTargetB = false;
// const uint8_t onTargetB  = 12;    // On Target B Light
volatile bool onTargetB  = false;
// const uint8_t shortLEDB  = 13;    // Short Circuit A Light

const u_int8_t LED_DATA_PIN = 18;
const u_int8_t LED_DATA_PIN_TWO =19; 
const u_int8_t groundPinA = 0;    // Ground A pin - Analog
const u_int8_t weaponPinA = 15;    // Weapon A pin - Analog
const u_int8_t lamePinA   = 2;    // Lame   A pin - Analog (Epee return path)
const u_int8_t lamePinB   = 34;    // Lame   B pin - Analog (Epee return path)
const u_int8_t weaponPinB = 35;    // Weapon B pin - Analog
const u_int8_t groundPinB = 32;    // Ground B pin - Analog

const u_int8_t modePin    =  5;        // Mode change button interrupt pin 0 (digital pin 2)
const u_int8_t buzzerPin  =  18;        // buzzer pin
const u_int8_t modeLeds[] = {13, 12, 14}; // LED pins to indicate weapon mode selected {f e s}
...
// mode constants
//=================
const u_int8_t FOIL_MODE  = 0;
const u_int8_t EPEE_MODE  = 1;
const u_int8_t SABRE_MODE = 2;
...
void setup() {
  ...
  FastLED.addLeds<WS2812B, LED_DATA_PIN, GRB>(RedFencerLEDs, NUM_LEDS_PER_STRIP);
  FastLED.addLeds<WS2812B, LED_DATA_PIN_TWO, GRB>(GreenFencerLEDs, NUM_LEDS_PER_STRIP);
...
void loop() {
  ...
  if(hitOnTargA) {
    fill_solid(RedFencerLEDs, NUM_LEDS_PER_STRIP, CRGB::Red);
    FastLED.show();
  }

  if(hitOffTargA) {
    fill_solid(RedFencerLEDs, NUM_LEDS_PER_STRIP, CRGB::Yellow);
    FastLED.show();
  }

  if(hitOnTargB) {
    fill_solid(GreenFencerLEDs, NUM_LEDS_PER_STRIP, CRGB::Green);
    FastLED.show();
  }

  if(hitOffTargB) {
    fill_solid(GreenFencerLEDs, NUM_LEDS_PER_STRIP, CRGB::Yellow);
    FastLED.show();
  }
  ...
  while (1) {
    ...
    Serial.println("Weapon A Value");
    Serial.println(weaponA);
    Serial.println("Lame B Value");
