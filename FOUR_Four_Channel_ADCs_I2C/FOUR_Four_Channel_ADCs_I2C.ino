...
void setup(void) {
  ...
  const int HIOffTarget = 2040; // 4.095V 32760 Convert the voltage to a 16bit value 0.000125V per bit
  const int LOOffTarget = 1640; // 3.28V 26240 Convert the voltage to a 16bit value 0.000125V per bit
  ...
  attachInterrupt(digitalPinToInterrupt(WeaponA_OFFTARGET_ALRT_PIN), ISR_OffTargetA, FALLING); 
  attachInterrupt(digitalPinToInterrupt(WeaponB_OFFTARGET_ALRT_PIN), ISR_OffTargetB, FALLING );
  Wire.begin();
...
void loop(void) {
  ...
  // Serial.print("RawValue_ADS_A: "); Serial.print("Alert/Ready pin = "); Serial.println(digitalRead(WeaponA_ONTARGET_ALRT_PIN)); //Display the voltage measured on A0 on the serial monitor
  // Serial.print("RawValue_ADS_B: "); Serial.print("Alert/Ready pin = "); Serial.println(digitalRead(WeaponB_ONTARGET_ALRT_PIN)); //Display the voltage measured on A0 on the serial monitor
